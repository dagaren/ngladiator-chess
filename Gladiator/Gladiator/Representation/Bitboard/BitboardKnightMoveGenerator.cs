...
{
    class BitboardKnightMoveGenerator<TPosition> : IMoveGenerator<TPosition, BitboardBoard> where TPosition : IPosition<BitboardBoard>
    {
        public IList<Move> GetMoves(TPosition position)
        {
            IList<Move> movesList = new List<Move>();

            ColouredPiece colouredPiece = Piece.Knight.GetColoured(position.Turn);
            ...
            while(pieceBitboard != BitboardExtensions.Empty)
            {
                ...
                ulong attackedBitboard = KnightBitboards.AttackBitboards[source.GetValue()];
